...
def simulation():
    ...
    print(f"~Keeping the first choice resulted in {keep_choice_wins} wins out of {iterations} games, or {str(keep_choice_wins / iterations * 100)[:5]}% of the time")
    print(f"Changing the first choice resulted in {change_choice_wins} wins out of {iterations} games, or {str(change_choice_wins / iterations * 100)[:5]}% of the time")
